...
- Object bytes are written under `storage/` (or `STORAGE_DIR` env override), organized using the object key's path structure with a `{YYYY}/{MM}/{DD}` date folder inserted before the filename / last path segment (or after the prefix for deeper paths) and a random UUID filename (preserving the file extension) — stored in `objects.relative_storage_path`. Uploads land in `storage/{bucket}/.tmp/` first and are `rename()`d into place, so readers never observe a partially-written object.
