...
- Object bytes are written under `storage/` (or `STORAGE_DIR` env override), organized using the object key's full directory path with a `{YYYY}/{MM}/{DD}` date folder inserted immediately before the random UUID filename (preserving the file extension) regardless of directory depth — stored in `objects.relative_storage_path`. Uploads land in `storage/{bucket}/.tmp/` first and are `rename()`d into place, so readers never observe a partially-written object.
